...
class HGT(nn.Module):
    ...
    @torch.no_grad()
    def ue_norm_attn_computer(self, user_emb, entity_emb, edge_index):
        head, tail = edge_index

        query = (user_emb[head] @ self.W_Q).view(-1, self.n_heads, self.d_k)
        key = (entity_emb[tail] @ self.W_Q).view(-1, self.n_heads, self.d_k)
        ...
        edge_attn_score = scatter_softmax(edge_attn_logits, head)
        # 除以head node的度进行归一化
        norm = scatter_sum(torch.ones_like(head), head, dim=0, dim_size=user_emb.shape[0])
        norm = torch.index_select(norm, 0, head)
        edge_attn_score = edge_attn_score * norm

...
def _adaptive_ui_drop_cl(item_attn_mean, inter_edge, inter_edge_w, keep_rate=0.7):
    """ random drop """
    # cl_kg_mask = torch.ones_like(inter_edge_w).bool()
    # cl_kg_mask[torch.randperm(inter_edge_w.shape[0])[:int((1-keep_rate) * inter_edge_w.shape[0])]] = False
    # return inter_edge[:, cl_kg_mask], inter_edge_w[cl_kg_mask]

    inter_attn_prob = item_attn_mean[inter_edge[1]]
    # print(inter_attn_prob[:100])
    # add gumbel noise
    noise = -torch.log(-torch.log(torch.rand_like(inter_attn_prob)))
    """ least hard drop """
    # inter_attn_prob = inter_attn_prob + noise
    # _, least_attn_item_id = torch.topk(-inter_attn_prob,
    #                                    int((1-keep_rate) * inter_attn_prob.shape[0]), sorted=False)
    # cl_kg_mask = torch.ones_like(inter_edge_w).bool()
    # cl_kg_mask[least_attn_item_id] = False
    # return inter_edge[:, cl_kg_mask], inter_edge_w[cl_kg_mask]
    """ prob based drop """
    inter_attn_prob = inter_attn_prob + noise
    # inter_attn_prob = torch.sigmoid(inter_attn_prob)
    """hard drop"""
    # softmax基数太大导致随机问题?
    ...
    # torch sample
    # inter_attn_prob[inter_attn_prob < 0.] = 0.
    sampled_edge_idx = torch.multinomial(inter_attn_prob, int(keep_rate * inter_edge_w.shape[0]), replacement=False)

    # point-based random sampling
    # inter_attn_prob = inter_attn_prob * keep_rate / inter_attn_prob.mean()
    # inter_attn_prob[inter_attn_prob > 1.] = 1.
    # sample_mask = torch.bernoulli(inter_attn_prob)
    # sampled_edge_idx = torch.nonzero(sample_mask).squeeze()
    return inter_edge[:, sampled_edge_idx], inter_edge_w[sampled_edge_idx]/keep_rate
    """soft drop"""
    # inter_attn_prob = inter_attn_prob * (keep_rate / inter_attn_prob.mean())
    # return inter_edge, inter_edge_w * inter_attn_prob
...
def _mae_edge_mask_random(edge_index, edge_type, mask_size):
    n_edges = edge_index.shape[1]
    mask = np.zeros(n_edges, dtype=np.bool)
    random_indices = np.random.choice(
        n_edges, size=mask_size, replace=False)
...
class KGSR(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, hp_dict=None):
        ...
        self.grace_loss = GRACE(self.emb_size, tau=self.tau)
    ...
    def forward(self, batch=None):
        ...
        if self.ablation == "m_rand":
            enc_edge_index, enc_edge_type, masked_edge_index, masked_edge_type, mask_bool = _mae_edge_mask_random(edge_index, edge_type, 2*topk_attn_edge_id.shape[0])
        else:
        ...
        if self.ablation == "no_mae":
            # create zero tensor
            mae_loss = torch.zeros(1).to(self.device)
        else:
            # MAE task
            # mask_size, 2, channel
        ...
        if self.ablation == "no_cl":
            cl_loss = torch.zeros(1).to(self.device)
        else:
            # CL task
            if self.ablation == "cl_rand":
                """random way of sampling"""
                cl_ui_edge, cl_ui_w = _sparse_dropout(inter_edge, inter_edge_w, keep_rate=0.4)
                cl_kg_edge, cl_kg_type = _relation_aware_edge_sampling(edge_index, edge_type, self.n_relations, samp_rate=0.4)
            else:
                """adaptive sampling"""
                ...
                cl_ui_edge, cl_ui_w = _adaptive_ui_drop_cl(
                    item_attn_mean, inter_edge, inter_edge_w, 1-self.cl_drop)

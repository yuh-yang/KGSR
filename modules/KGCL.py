...
class KGCL(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat):
        ...
        self.adj_mat = self._convert_sp_mat_to_sp_tensor(adj_mat).to(self.device)
        self.graph = graph
        self.edge_index, self.edge_type = self._get_edges(graph)

        self.all_embed = nn.init.normal_(torch.empty(self.n_nodes, self.emb_size), std=0.1)
        self.relation_embed = nn.init.normal_(torch.empty(self.n_relations, self.emb_size), std=0.1)
    ...
    def get_ui_aug_views(self, kg_stability, mu):
        ...
        out = torch.sparse.FloatTensor(i, v, self.adj_mat.shape).to(self.adj_mat.device)
    ...
    def gcn(self, edge_index, edge_type, g):
        ...
        entity_emb = self.rgat(entity_emb, self.relation_embed, [edge_index, edge_type], self.mess_dropout&self.training)
    ...
    def generate(self):
        return self.gcn(self.edge_index, self.edge_type, self.adj_mat)

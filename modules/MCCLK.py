...
class Recommender(nn.Module):
    ...
    def forward(
        ...
        batch_item = torch.cat((pos_item, neg_item))
        ...
        i_e_2 = item_lightgcn_emb[batch_item]
        ...
        i_pos = i_e[:pos_item.shape[0]]
        i_neg = i_e[pos_item.shape[0]:]

        return self.create_bpr_loss(u_e, i_pos, i_neg, loss_contrast)
    ...
    def create_bpr_loss(self, users, pos_items, neg_items, loss_contrast):
        ...
        regularizer = (torch.norm(users) ** 2
                       + torch.norm(torch.cat((pos_items,neg_items))) ** 2) / 2
        emb_loss = self.decay * regularizer / batch_size
        # cor_loss = self.sim_decay * cor
        return mf_loss + emb_loss + 0.001*loss_contrast, pos_scores, mf_loss, emb_loss
    ...
    def generate(self):
        ...
        user_lightgcn_emb, item_lightgcn_emb = self.light_gcn(user_emb, item_emb, interact_graph)
        u_e = torch.cat((user_gcn_emb, user_lightgcn_emb, user_lightgcn_emb), dim=-1)
        i_e = torch.cat((entity_gcn_emb, i_h, item_lightgcn_emb), dim=-1)
        return i_e, u_e
